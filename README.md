# This is a fork from [trusted-sound-source-localization](https://github.com/Devin-Pi/trusted-sound-source-localization.git) for experimental use. Please clone the original repository for your reference.
...
```zsh
python data_simu.py DATA_SIMU.TRAIN=True
```
```zsh
python data_simu.py DATA_SIMU.DEV=True
```
```zsh
python data_simu.py DATA_SIMU.TEST=True
```
...
- **Model Training**
```zsh
python main.py fit --config /workspaces/audio_deep_learning_1/config/tcrnn.yaml
```
...
```zsh
python main.py test --config /workspaces/audio_deep_learning_1/config/tcrnn.yaml
```
